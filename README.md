...
| 入力 | 出力 |
| --- | --- |
| 困りごと（自由文） | **注意すべき論点5つ** — 回答の一番上。この相談者に効く順に、1行ずつ |
| | **見る順番** — priority 順の資料リスト（JETRO解説 → JBIC投資環境 → 一次資料） |
...
- `data/topics.json` に論点を追加。`cautions`（短い注意点2つ）は回答冒頭の「注意すべき論点5つ」に使われる。`keywords` は相談者が実際に書きそうな口語（「人を雇いたい」「店を出したい」）を入れる。
